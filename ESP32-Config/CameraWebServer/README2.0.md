...
```bash
cat > ~/detect.py << 'EOF'
import cv2, numpy as np, time, threading, websocket
from ultralytics import YOLO

CAM_IP      = "192.168.50.18"          # << your ESP32-CAM IP from section 06
...
def reader():
    """Read WS frames as fast as the camera ships, always overwriting
    the single-slot buffer. Stale frames get dropped automatically."""
    while True:
        try:
...
def main():
    ...
    while True:
        ...
        last = now
        cv2.putText(annotated, f"{fps:5.1f} FPS  |  {len(results[0].boxes)} det",
